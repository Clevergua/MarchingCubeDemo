    ...
    public class IslandGenerator
    {
        ...
        public async Task<Island> GenerateAsync(int seed, int horizontalChunkCount)
        {
            ...
            Debug.Log(currentOperation);
            {
                ...
                await Task.Run(() => { GenerateCoord2Shape(c2s, coord2NoiseFactor, environmentmap, length, seed); });
                coord2Shape = c2s;
            }
            progress = 40;
            ...
            {
                await Task.Run(() => { FillBlockmapByBaseHeight(blockmap, environmentmap, length); });
                await Task.Run(() => { ShapeBlockmap(blockmap, coord2Shape, environmentmap, length, height, seed); });
            }
            progress = 60;
            currentOperation = "正在生成地表";
            Debug.Log(currentOperation);
            {
                await Task.Run(() => { GenerateSurface(blockmap, environmentmap, coord2NoiseFactor, seed); });
            }
            progress = 70;
        ...
        private void FillBlockmapByBaseHeight(byte[,,] blockmap, Environmentmap environmentmap, int length)
        {
            for (int x = 0; x < length; x++)
            {
        ...
        private void GenerateCoord2Shape(Dictionary<Coord3Int, bool> coord2Shape, IReadOnlyDictionary<Coord3Int, float> coord2NoiseFactor, Environmentmap environmentmap, int length, int seed)
        {
            var density = 0.023f;
            var concave = false;
            var convex = true;
        ...
        private void ShapeBlockmap(byte[,,] blockmap, IReadOnlyDictionary<Coord3Int, bool> coord2Shape, Environmentmap environmentmap, int length, int height, int seed)
        {
            ...
            for (int x = 0; x < length; x++)
            {
                for (int z = 0; z < length; z++)
                {
                    for (int y = 0; y < height; y++)
                    {
                        ...
                        {
                            ProcessShape(blockmap, processedCoords, coord2Shape, startCoord, environmentmap, length, height, seed);
                        }
                    }
                }
            }
        }
        private void ProcessShape(byte[,,] blockmap, HashSet<Coord3Int> processedCoords, IReadOnlyDictionary<Coord3Int, bool> coord2Shape, Coord3Int startCoord, Environmentmap environmentmap, int length, int height, int seed)
        {
            //如果此点是某种形状
        ...
        private void GenerateSurface(byte[,,] blockmap, Environmentmap environmentmap, IReadOnlyDictionary<Coord3Int, float> coord2NoiseFactor, int seed)
        {
